...
class ShandongWeatherVisualization:
    ...
    def calculate_suitability(self, current_temp, min_temp, max_temp):
        ...
        try:
            ...
            if current_temp < min_temp:
                ...
                # 对于低温，允许更大的偏差范围
                suitability = max(0, 100 * (1 - deviation / 8))
            elif current_temp > max_temp:
                # 如果温度高于最高适宜温度
                deviation = abs(current_temp - max_temp)
                suitability = max(0, 100 * (1 - deviation / 5))
            else:
                # 在适宜范围内
                ...
                suitability = 100 * (1 - deviation * 0.3)
    ...
    def draw_temperature_legend(self):
        ...
        temp_ranges = [
            ("较暖 1-2°C", "#ffa726", legend_y + 35),
            ("温和 -1-1°C", "#4ecdc4", legend_y + 60),
            ("较冷 -2--1°C", "#45b7d1", legend_y + 85),
            ("寒冷 -3--2°C", "#3498db", legend_y + 110)
        ]
    ...
    def get_temperature_color(self, temp):
        """根据温度获取颜色 - 适应-3到2度范围"""
        if temp >= 1:
            return "#ffa726"  # 橙色 - 相对较暖
        elif temp >= -1:
            return "#4ecdc4"  # 青色 - 温和
        elif temp >= -2:
            return "#45b7d1"  # 浅蓝色 - 较冷
        else:
            return "#3498db"  # 蓝色 - 寒冷
    ...
    def get_mock_weather_data(self, city_name):
        """生成模拟天气数据 - 温度控制在-3到2摄氏度之间"""
        # 为不同城市设置基础温度偏移，但总体范围控制在-3到2度
        city_temp_offset = {
            "济南": 0, "青岛": 0.5, "淄博": 0, "枣庄": 1, "东营": -0.5,
            "烟台": 0.2, "潍坊": 0, "济宁": 0.5, "泰安": -0.5, "威海": 0.3,
            "日照": 0.5, "临沂": 1, "德州": -0.8, "聊城": 0, "滨州": -0.5, "菏泽": 0.5
        }
        
        # 控制在-3到2度的范围
        base_temp = random.uniform(-3, 2)
        offset = city_temp_offset.get(city_name, 0)
        
        # 应用城市偏移，但仍保持在范围内
        temp = max(-3, min(2, base_temp + offset * 0.3))
        
        weather_types = ['晴', '多云', '阴', '小雨', '小雪', '雨夹雪']
        
        # 根据温度调整天气类型概率
        if temp < 0:
            # 低于0度时，增加小雪和雨夹雪的概率
            weather_weights = [0.3, 0.3, 0.2, 0.1, 0.1, 0.1]
        else:
            weather_weights = [0.4, 0.3, 0.2, 0.3, 0.0, 0.0]
        
        weather_text = random.choices(weather_types, weights=weather_weights, k=1)[0]
        
        # 体感温度通常比实际温度略低，特别是在低温天气
        if temp < 0:
            feels_like = max(-5, min(1, temp - random.uniform(0.5, 2)))
        else:
            feels_like = max(-3, min(2, temp - random.uniform(0, 1)))
        
        # 其他天气参数也根据低温天气调整
        if temp < 0:
            humidity = random.randint(40, 70)  # 低温天气湿度通常较低
            wind_speed = random.uniform(10, 25)  # 冬季风力可能较大
            wind_scale = random.randint(2, 4)
        else:
            humidity = random.randint(50, 85)
            wind_speed = random.uniform(5, 15)
            wind_scale = random.randint(1, 3)
        
        pressure = random.randint(1020, 1040)  # 冬季气压通常较高
        precip = random.uniform(0, 1) if temp < 0 else random.uniform(0, 2)
        vis = random.uniform(5, 20)  # 冬季能见度可能较低
        ...
        return {
            ...
            "now": {
                ...
                "text": weather_text,
                "windDir": random.choice(["北风", "西北风", "东北风", "偏北风"]),
